...
def extract_author_info(config):
    ...
    if 'author' in config:
        ...
        if author.get('employer'):
            employer = author.get('employer')
            if isinstance(employer, list):
                # Format like: employer1, employer2, and employerN
                if len(employer) == 1:
                    employers_str = employer[0]
                elif len(employer) == 2:
                    employers_str = f"{employer[0]} and {employer[1]}"
                else:
                    employers_str = ", ".join(employer[:-1]) + f", and {employer[-1]}"
            else:  # Just a string
                employers_str = employer
            author_info['summary'] = f"Currently employed at {employers_str}"
